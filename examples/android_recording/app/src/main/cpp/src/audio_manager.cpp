...
AudioManager::AudioManager()
    : j_environment_(JVM::GetInstance()->environment()),
      audio_layer_(AudioLayer::kPlatformDefaultAudio),
      initialized_(false),
      hardware_aec_(false),
      hardware_agc_(false),
      hardware_ns_(false),
  ...
  JNINativeMethod native_methods[] = {
      {"nativeCacheAudioParameters", "(IIIZZZZZZZIIJ)V",
       reinterpret_cast<void *>(&awaudio::AudioManager::CacheAudioParameters)}};
  j_native_registration_ = j_environment_->RegisterNatives(
      "com/example/audiorecording/AwAudioManager", native_methods,
...
//  RTC_DCHECK(thread_checker_.IsCurrent());
  ...
  delay_estimate_in_milliseconds_ =
      (audio_layer == AudioLayer::kAndroidJavaAudio)
...
//  RTC_DCHECK(thread_checker_.IsCurrent());
  RTC_DCHECK(!initialized_);
//  RTC_DCHECK_NE(audio_layer_, AudioLayer::kPlatformDefaultAudio);
...
void JNICALL AudioManager::CacheAudioParameters(JNIEnv *env,
  ...
  awaudio::AudioManager *this_object =
      reinterpret_cast<awaudio::AudioManager *>(native_audio_manager);
