...
public:
  explicit ScopedHistogramTimer(const std::string &name)
      : histogram_name_(name), start_time_ms_(awaudio::TimeMillis()) {}

  ~ScopedHistogramTimer() {
    const int64_t life_time_ms = awaudio::TimeSince(start_time_ms_);
  ...
  JNINativeMethod native_methods[] = {
      {"nativeCacheDirectBufferAddress", "(Ljava/nio/ByteBuffer;J)V",
          reinterpret_cast<void *>(
              &awaudio::AudioRecordJni::CacheDirectBufferAddress)},
      {"nativeDataIsRecorded",           "(IJ)V",
          reinterpret_cast<void *>(&awaudio::AudioRecordJni::DataIsRecorded)}};
  j_native_registration_ = j_environment_->RegisterNatives(
      "com/example/audiorecording/AwAudioRecord", native_methods,
...
//  RTC_DCHECK(thread_checker_.IsCurrent());
  ...
  if (frames_per_buffer < 0) {
    direct_buffer_address_ = nullptr;
    RTC_LOG(LS_ERROR) << "InitRecording failed " << frames_per_buffer;
    return -1;
  }
...
void AudioRecordJni::SetDataCallback(AudioRecorderDataCallback *data_callback) {
  RTC_LOG(LS_INFO) << "SetDataCallback()";
//  RTC_DCHECK(thread_checker_.IsCurrent());
...
void JNICALL AudioRecordJni::CacheDirectBufferAddress(JNIEnv *env,
  ...
  awaudio::AudioRecordJni *this_object =
      reinterpret_cast<awaudio::AudioRecordJni *>(nativeAudioRecord);
...
//  RTC_DCHECK(thread_checker_java_.IsCurrent());
  if (!audio_data_callback_) {
    RTC_LOG(LS_ERROR) << "SetDataCallback has not been called";
    return;
  }
  ...
  int32_t ret = audio_data_callback_->DeliverRecordedData(direct_buffer_address_,
                                                          frames_per_buffer_,
                                                          AUDIO_FORMAT_PCM_I16, channels,
                                                          sample_rate_hz,
                                                          total_delay_in_milliseconds_,
                                                          newMicLevel);
  if (ret == -1) {
