...
GtkMainWnd::VideoRenderer::VideoRenderer(
    GtkMainWnd *main_wnd, webrtc::VideoTrackInterface *track_to_render)
...
void GtkMainWnd::VideoRenderer::OnFrame(const webrtc::VideoFrame &video_frame) {
  ...
  std::size_t date_len = std::strftime(buf, sizeof(buf), "%Y-%m-%d %H:%M:%S",
                                       std::localtime(&now_tm_t));
  auto millis = std::chrono::duration_cast<std::chrono::milliseconds>(
      now.time_since_epoch());
  snprintf(buf + date_len, sizeof(buf) - date_len, ".%03d",
