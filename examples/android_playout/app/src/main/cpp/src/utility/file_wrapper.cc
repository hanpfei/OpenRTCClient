...
FILE* FileOpen(const char* file_name_utf8, bool read_only, int* error) {
  ...
  if (!file) {
    RTC_LOG(LS_INFO) << "Open file " << file_name_utf8 << " failed for " << strerror(errno);
    if (error) {
      *error = errno;
    }
  }
...
const char* GetCstrCheckNoEmbeddedNul(const std::string& s) {
  ...
  if (strlen(p) != s.size()) {
    RTC_LOG(LS_INFO) << "Invalid filename, containing NUL character";
  }
  return p;
}
}  // namespace
...
bool FileWrapper::SeekRelative(int64_t offset) {
  RTC_DCHECK(file_);
  return fseek(file_, static_cast<long>(offset), SEEK_CUR) == 0;
}
