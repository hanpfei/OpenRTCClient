...
#define RTC_EAT_STREAM_PARAMETERS(ignored)                          \
  (true ? true : ((void)(ignored), true))                           \
      ? static_cast<void>(0)                                        \
      : ::awaudio::webrtc_checks_impl::FatalLogCall<false>("", 0, "") & \
            ::awaudio::webrtc_checks_impl::LogStreamer<>()

// Call RTC_EAT_STREAM_PARAMETERS with an argument that fails to compile if
// values of the same types as `a` and `b` can't be compared with the given
// operation, and that would evaluate `a` and `b` if evaluated.
#define RTC_EAT_STREAM_PARAMETERS_OP(op, a, b) \
  RTC_EAT_STREAM_PARAMETERS(((void)::awaudio::Safe##op(a, b)))
...
#define RTC_CHECK(condition)                                    \
  (condition) ? static_cast<void>(0)                            \
              : ::awaudio::webrtc_checks_impl::FatalLogCall<false>( \
                    __FILE__, __LINE__, #condition) &           \
                    ::awaudio::webrtc_checks_impl::LogStreamer<>()

#define RTC_CHECK_OP(name, op, val1, val2)                 \
  ::awaudio::Safe##name((val1), (val2))                        \
      ? static_cast<void>(0)                               \
      : ::awaudio::webrtc_checks_impl::FatalLogCall<true>(     \
            __FILE__, __LINE__, #val1 " " #op " " #val2) & \
            ::awaudio::webrtc_checks_impl::LogStreamer<>() << (val1) << (val2)
#else
#define RTC_CHECK(condition)                                                  \
  (condition)                                                                 \
      ? static_cast<void>(0)                                                  \
      : true ? ::awaudio::webrtc_checks_impl::FatalLogCall<false>(__FILE__,       \
                                                              __LINE__, "") & \
                   ::awaudio::webrtc_checks_impl::LogStreamer<>()                 \
             : ::awaudio::webrtc_checks_impl::FatalLogCall<false>("", 0, "") &    \
                   ::awaudio::webrtc_checks_impl::LogStreamer<>()
...
#define RTC_CHECK_OP(name, op, val1, val2)                                   \
  ::awaudio::Safe##name((val1), (val2))                                          \
      ? static_cast<void>(0)                                                 \
      : true ? ::awaudio::webrtc_checks_impl::FatalLogCall<true>(__FILE__,       \
                                                             __LINE__, "") & \
                   ::awaudio::webrtc_checks_impl::LogStreamer<>()                \
             : ::awaudio::webrtc_checks_impl::FatalLogCall<false>("", 0, "") &   \
                   ::awaudio::webrtc_checks_impl::LogStreamer<>()
...
#define RTC_CHECK_NOTREACHED()                         \
  do {                                                 \
    ::awaudio::webrtc_checks_impl::UnreachableCodeReached( \
        RTC_UNREACHABLE_FILE_AND_LINE_CALL_ARGS);      \
  } while (0)

#define RTC_FATAL()                                                  \
  ::awaudio::webrtc_checks_impl::FatalLogCall<false>(__FILE__, __LINE__, \
                                                 "FATAL()") &        \
      ::awaudio::webrtc_checks_impl::LogStreamer<>()
