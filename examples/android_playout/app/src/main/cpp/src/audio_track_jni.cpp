...
AudioTrackJni::JavaAudioTrack::JavaAudioTrack(
    ...
    : audio_track_(std::move(audio_track)),
      ...
      set_stream_type_(native_reg->GetMethodId("setStreamType", "(I)Z")),
...
bool AudioTrackJni::JavaAudioTrack::SetStreamType(int streamType) {
  return audio_track_->CallBooleanMethod(set_stream_type_, streamType);
}
  ...
  JNINativeMethod native_methods[] = {
      {"nativeCacheDirectBufferAddress", "(Ljava/nio/ByteBuffer;J)V",
          reinterpret_cast<void *>(
              &awaudio::AudioTrackJni::CacheDirectBufferAddress)},
      {"nativeGetPlayoutData",           "(IJ)V",
          reinterpret_cast<void *>(&awaudio::AudioTrackJni::GetPlayoutData)}};
  j_native_registration_ = j_environment_->RegisterNatives(
      "com/example/audioplayout/AwAudioTrack", native_methods,
...
//  RTC_DCHECK(thread_checker_.IsCurrent());
  RTC_DCHECK(!playing_);
  if (!initialized_) {
    RTC_LOG(LS_WARNING) << "Playout can not start since InitPlayout must succeed first";
    return 0;
  }
...
int AudioTrackJni::SetSpeakerStreamType(int streamType) {
  RTC_LOG(LS_INFO) << "SetSpeakerStreamType(" << streamType << ")";
//  RTC_DCHECK(thread_checker_.IsCurrent());
  return j_audio_track_->SetStreamType(streamType) ? 0 : -1;
}
...
void AudioTrackJni::SetDataCallback(AudioPlayerDataCallback *data_callback) {
  RTC_LOG(LS_INFO) << "AttachAudioBuffer()";
//  RTC_DCHECK(thread_checker_.IsCurrent());
  audio_data_callback_ = data_callback;
  const int sample_rate_hz = audio_parameters_.sample_rate();
  RTC_LOG(LS_INFO) << "SetPlayoutSampleRate(" << sample_rate_hz << ")";
  const size_t channels = audio_parameters_.channels();
  RTC_LOG(LS_INFO) << "SetPlayoutChannels(" << channels << ")";
...
void JNICALL AudioTrackJni::CacheDirectBufferAddress(JNIEnv *env,
  ...
  awaudio::AudioTrackJni *this_object =
      reinterpret_cast<awaudio::AudioTrackJni *>(nativeAudioTrack);
...
//  RTC_DCHECK(thread_checker_java_.IsCurrent());
  ...
  const size_t channels = audio_parameters_.channels();
  const int sample_rate_hz = audio_parameters_.sample_rate();
  int samples = audio_data_callback_->RequestPlayoutData(frames_per_buffer_, AUDIO_FORMAT_PCM_I16,
  ...
  samples = audio_data_callback_->GetPlayoutData(direct_buffer_address_);
