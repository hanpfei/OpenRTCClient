...
public class AwAudioTrack {
    private static final boolean DEBUG = false;

    private static final String TAG = "com.example.audioplayout.AwAudioTrack";
    ...
    // By default, AW creates audio tracks with a usage attribute
    ...
    @Deprecated
    public static interface AwAudioTrackErrorCallback {
        void onAwAudioTrackInitError(String errorMessage);

        void onAwAudioTrackStartError(String errorMessage);

        void onAwAudioTrackError(String errorMessage);
    }
    ...
    public static interface ErrorCallback {
        ...
        void onAwAudioTrackStartError(AudioTrackStartErrorCode errorCode, String errorMessage);

        void onAwAudioTrackError(String errorMessage);
    }

    private static @Nullable AwAudioTrackErrorCallback errorCallbackOld;
    private static @Nullable ErrorCallback errorCallback;

    @Deprecated
    public static void setErrorCallback(AwAudioTrackErrorCallback errorCallback) {
        Logging.d(TAG, "Set error callback (deprecated");
        AwAudioTrack.errorCallbackOld = errorCallback;
    }

    public static void setErrorCallback(ErrorCallback errorCallback) {
        Logging.d(TAG, "Set extended error callback");
        AwAudioTrack.errorCallback = errorCallback;
    }
    ...
    private class AudioTrackThread extends Thread {
        ...
        public void run() {
            ...
            Logging.d(TAG, "AudioTrackThread" + AwAudioUtils.getThreadInfo());
            assertTrue(audioTrack.getPlayState() == AudioTrack.PLAYSTATE_PLAYING);

            // Fixed size in bytes of each 10ms block of audio data that we ask for
            // using callbacks to the native WebRTC client.
            final int sizeInBytes = byteBuffer.capacity();
            Logging.w(TAG, "Stream type " + audioTrack.getStreamType());
            while (keepAlive) {
                ...
                if (bytesWritten != sizeInBytes) {
                    ...
                    if (bytesWritten < 0) {
                        keepAlive = false;
                        reportAwAudioTrackError("AudioTrack.write failed: " + bytesWritten);
                    }
                }
    ...
    AwAudioTrack(long nativeAudioTrack) {
    ...
    private int initPlayout(int sampleRate, int channels, double bufferSizeFactor) {
        ...
        if (minBufferSizeInBytes < byteBuffer.capacity()) {
            reportAwAudioTrackInitError("AudioTrack.getMinBufferSize returns an invalid value.");
            return -1;
        }
        ...
        if (audioTrack != null) {
            reportAwAudioTrackInitError("Conflict with existing AudioTrack.");
            return -1;
        }
        try {
            ...
            if (Build.VERSION.SDK_INT >= 21) {
                ...
                audioTrack =
                        createAudioTrackOnLowerThanLollipop(sampleRate, channelConfig,
                                minBufferSizeInBytes, streamType);
            }
        } catch (IllegalArgumentException e) {
            reportAwAudioTrackInitError(e.getMessage());
        ...
        if (audioTrack == null || audioTrack.getState() != AudioTrack.STATE_INITIALIZED) {
            reportAwAudioTrackInitError("Initialization of audio track failed.");
            releaseAudioResources();
            return -1;
        }
        logMainParameters();
    ...
    private boolean setStreamType(int streamType) {
        threadChecker.checkIsOnValidThread();
        Logging.w(TAG, "setStreamType(" + streamType + ")");
        assertTrue(audioManager != null);
        this.streamType = streamType;
        if (streamType == AudioManager.STREAM_VOICE_CALL) {
            usageAttribute = DEFAULT_USAGE;
        } else if (streamType == AudioManager.STREAM_MUSIC) {
            usageAttribute = AudioAttributes.USAGE_MEDIA;
        }
        return true;
    }
    ...
    private boolean setStreamVolume(int volume) {
        threadChecker.checkIsOnValidThread();
        Logging.w(TAG, "setStreamVolume(" + volume + ")");
        assertTrue(audioManager != null);
        if (isVolumeFixed()) {
    ...
    private void reportAwAudioTrackInitError(String errorMessage) {
        Logging.e(TAG, "Init playout error: " + errorMessage);
        AwAudioUtils.logAudioState(TAG);
        if (errorCallbackOld != null) {
            errorCallbackOld.onAwAudioTrackInitError(errorMessage);
        }
        if (errorCallback != null) {
            errorCallback.onAwAudioTrackInitError(errorMessage);
        }
    }

    private void reportAwAudioTrackStartError(
            AudioTrackStartErrorCode errorCode, String errorMessage) {
        Logging.e(TAG, "Start playout error: " + errorCode + ". " + errorMessage);
        AwAudioUtils.logAudioState(TAG);
        if (errorCallbackOld != null) {
            errorCallbackOld.onAwAudioTrackStartError(errorMessage);
        }
        if (errorCallback != null) {
            errorCallback.onAwAudioTrackStartError(errorCode, errorMessage);
        }
    }

    private void reportAwAudioTrackError(String errorMessage) {
        Logging.e(TAG, "Run-time playback error: " + errorMessage);
        AwAudioUtils.logAudioState(TAG);
        if (errorCallbackOld != null) {
            errorCallbackOld.onAwAudioTrackError(errorMessage);
        }
        if (errorCallback != null) {
            errorCallback.onAwAudioTrackError(errorMessage);
        }
    }
}
