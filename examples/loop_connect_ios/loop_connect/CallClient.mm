...
CallClient::CallClient() : call_started_(false) {
  thread_checker_.Detach();
  CreatePeerConnectionFactory();
}

void CallClient::CreatePeerConnectionFactory() {
...
bool CallClient::LoopCallStarted() {
  webrtc::MutexLock lock(&pc_mutex_);
  return call_started_;
}

void CallClient::CreateConnections() {
  webrtc::MutexLock lock(&pc_mutex_);
...
void CallClient::StartLoopCall(RTC_OBJC_TYPE(RTCVideoCapturer) * capturer,
  ...
  rtc::scoped_refptr<webrtc::VideoTrackInterface> remote_video_track = recv_observer_->GetRemoteVideoTrack();
  remote_video_track->AddOrUpdateSink(remote_sink_.get(), rtc::VideoSinkWants());
}

void CallClient::DestroyConnections() {
  webrtc::MutexLock lock(&pc_mutex_);
